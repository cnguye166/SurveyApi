...
namespace Service
{
    internal sealed class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<string?> ValidateUserAndReturnIdAsync(UserForAuthenticationDto userForAuth)
        {
            _user = await _userManager.FindByNameAsync(userForAuth.UserName);
            var result = (_user != null && await _userManager.CheckPasswordAsync(_user, userForAuth.Password));
            if (!result)
            {
                _logger.LogWarn($"{nameof(ValidateUserAndReturnIdAsync)}: Authentication failed. Wrong username or password.");
                return null;
            }
        ...
        {
            var key = Encoding.UTF8.GetBytes("SurveyApiSecretKey");
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            ...
            var tokenValidationParameters = new TokenValidationParameters
            {
                ...
                IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("SurveyApiSercretKey")),
